...
1. Uses two GraphQL queries to retrieve the student's classes (sections) and some class details for today. Note, the date is passed so that today is determined by the browser's timezone rather than the server's timezone. The server timezone for Lambda is GMT/Zulu.
