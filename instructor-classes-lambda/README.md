...
1. Uses two GraphQL queries to retrieve the student's classes (sections) and some class details for today. Note, the date is passed in so that today is determined by the browser's timezone rather than the server's timezone. The server timezone for Lambda is GMT/Zulu.
...
Login to Experience as a user with permission to use Card Management. Locate the Today's Classes - Lambda card. This card has two items to configure on step 3.
